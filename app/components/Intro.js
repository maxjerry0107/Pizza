...
import {View,  ImageBackground,StatusBar,Image,Platform,Text,TouchableOpacity,Linking, Dimensions} from 'react-native';
...
const styles = StyleSheet.create({
  btnStyle:{
    minHeight: 70,
    width:'75%', backgroundColor:'#f11', borderWidth:5, borderColor:'#f11', alignItems:'center', justifyContent:'center',
  },
  btnTextStyle:{
    fontFamily:'Raleway-Bold', fontSize:RFPercentage(4), color:'white'
  }
});
  ...
  gotoLocation = () =>{
    ...
    render() {
      return (
        <View style={{flex:1}}>
          <StatusBar hidden />
          <Modal style={{flex:1, alignItems:'center', justifyContent:'center'}} backdropOpacity={0.8} isVisible={this.state.isPhonesign} onBackdropPress={()=>{
            this.setState({phonesign:false})
          }} >
            <View style={{width:'80%', flexDirection:'column', borderRadius:10,}} >
              <ImageBackground  source={require('../assets/images/intro/back1.png')} style={{width:'100%', alignItems:'center' , justifyContent:'center' }} imageStyle={{borderRadius:10, resizeMode:'cover'}}>
              <Image source={require('../assets/images/intro/intro_header.png')} style={{width:'90%',height:'40%'}} resizeMode={"contain"}/>
            <View style={{paddingBottom:20, paddingVertical:10, paddingHorizontal:20}}>
              <Text style={{fontSize:RFPercentage(3), alignSelf:'center', color:'#f00'}}>Phone Verification</Text>
              ...
              <View>
                <Text style={{fontSize:RFPercentage(2), alignSelf:'center'}}>We've sent code to your phone.</Text>
                <TextField 
                ...
                <View style={{flex:1, alignItems:'center', justifyContent:'center'}}>
                  <TouchableOpacity activeOpacity={0.7} onPress={()=>{
                    ...
                    else
                    {
                      ...
                      setTimeout(()=>{
                        this._sign_phonenum.setNativeProps({editable:true});
                        this._sign_phonenum.focus();
                      }, 200)
                    }
                  }}>
                    <View style={{ flexDirection: 'row' }}>
                      <Text style={{  color: '#939598',fontSize: RFPercentage(2.5), marginLeft: 3 }}>{"<"}</Text>
                      <Text style={{  color: '#f00',fontSize: RFPercentage(2.5), }}>Back</Text>
                    </View>
                  </TouchableOpacity>
                </View>
                <View style={{flex:1, alignItems:'center', justifyContent:'center',}}>
                  <TouchableOpacity activeOpacity={0.7} onPress={()=>{
                        ...
                        axios.post(url ,{
                          ...
                          .then(response => {
                           ...
                           //   this._sign_phonenum.setNativeProps({editable:false});   
                            ...
                            {
                              alert('Can\'t send verify code to your phonenum. Try again');
                            }
                          })
                          .catch(error=>{
                            alert('Can\'t send verify code to your phonenum. Try again');
                    ...
                    {             
                      if(this._sign_password.value()==this.state.verifyCode)
                      {
                    ...
                    <View style={{ flexDirection: 'row' }}>
                      <Text style={{  color: '#f00',fontSize: RFPercentage(2.5), }}>Next</Text>
                      <Text style={{  color: '#939598',fontSize: RFPercentage(2.5), marginLeft: 3 }}>></Text>
                    </View>
                  </TouchableOpacity>
                </View>
          ...
          <ImageBackground source={require('../assets/images/intro/back1.png')} style={{width:'100%', height:'100%'}}>
            <SafeAreaView style={[{flex:1}]} forceInset={{bottom:'always', top:'never'}}>
              ...
              <View style={{flex:1, flexDirection:'column',paddingTop:0,}}>
                <View style={{flex:1, alignContent:'center', alignItems:'center', paddingTop:0, zIndex:8, justifyContent:'center' }}>
                  <Image source={require('../assets/images/intro/intro_header.png')} resizeMode={'contain'} style={{height:'80%', width:'95%',zIndex:10}} ></Image>
                  <Image source={require('../assets/images/intro/satelite.png')} resizeMode={'contain'} style={{height:'90%', width:'58%',zIndex:8, position:'absolute', top:'65%', left:'-3%',}} ></Image>
                </View>
                <View style={{flex:2, zIndex:1,flexDirection:'column'}}>
                  ...
                  <View style={{flex:2, flexDirection:'column', width:'100%', alignItems:'center', justifyContent:'space-evenly', flexDirection:'column',}}>
                    <TouchableOpacity style={[styles.btnStyle,{height:screenWidth*0.13, }]} onPress={()=>{NavigationService.navigate('PickupSign')}} activeOpacity={0.8}>
                      <Text style={styles.btnTextStyle}>PICK UP</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.btnStyle,{height:screenWidth*0.13, }]} onPress={()=>{NavigationService.navigate('DeliverySign')}} activeOpacity={0.8}>
                      <Text style={styles.btnTextStyle}>DELIVERY</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.btnStyle,{height:screenWidth*0.13, }]}  onPress={()=>{NavigationService.navigate('OrderinSign')}} activeOpacity={0.8}>
