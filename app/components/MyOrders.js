...
export default class OrderTracking extends Component { 
  ...
  getSmokeOrder() {
    ...
    smoke_api.get('?req=get-orders&phonenum=' + global.myphonenumber)
      .then(response => {
          ...
          <ImageBackground source={require('../assets/images/sign/bg.png')} style={{width:'100%', height:'100%', resizeMode:'repeat'}}>
            <SafeAreaView  style={{width:'100%', flex:1, paddingTop:0,}}>
              <TouchableOpacity style={{width:'100%', height:RFPercentage(10),alignItems:'center', }} activeOpacity={0.8} onPress={()=>NavigationService.navigate("Intro")}>
                ...
                <Text style={{color:'#000',fontSize:RFPercentage(3), position:'absolute', bottom:0, fontWeight:'bold'}}>My Orders</Text>
              </TouchableOpacity>
{/* 
              ...
              <ScrollView style={{flex:1, flexDirection:'column', paddingTop:10,}}>
                {
                  this.state.orders.reverse().map ((item,key)=>{
                    return (
                      <View style={{flex:1,flexDirection:'column', marginBottom:10,backgroundColor:'rgba(255,255,255,0.6)', borderColor:'#888', borderBottomWidth:0.5, borderTopWidth:0.5}}  key={key} >
                      ...
                      <TouchableOpacity 
                        ...
                        <View style={{flexDirection:'column', justifyContent:'center',}}>
                          <Text style={{textAlign:'left', fontSize:RFPercentage(2.8),}}> Order ID - {item.orders_id}</Text>
                          <Text style={{textAlign:'left', fontSize:RFPercentage(2.4),}}> {item.created_on} </Text>
                        </View>
                        <View style={{flexDirection:'column', justifyContent:'center', marginRight:20, flex:1}}>
                          <Text style={{textAlign:'right', fontSize:RFPercentage(2.4), }}> {capitalizeFirstLetter(item.order_type)} </Text>
                          <Text style={{textAlign:'right',fontSize:RFPercentage(2.8)}}> ${item.order_total} </Text>
                        </View>
                      </TouchableOpacity>
                      <View>
                      {
                        item[0].map ((product_item,key1)=>{
                          return (
                            <TouchableOpacity key={"item"+item.orders_id+key1} style={{flexDirection:'row', paddingHorizontal:10, borderTopWidth:0.3, paddingVertical:5,  justifyContent:'center' }}  activeOpacity={0.7} 
                                 onPress={()=>NavigationService.navigate("Product",{product_id:product_item.products_id, type:product_item.product_type})}>
                              <Image source={{uri:product_item.img}} style={{width:'20%',aspectRatio:1}}></Image>
                              <View style={{flex:1, flexDirection:'column', paddingHorizontal:10, justifyContent:'center'}}>
                                  <Text style={{fontSize:RFPercentage(2.4), }}> {capitalizeFirstLetter(product_item.name)} </Text> 
                                <Text style={{fontSize:RFPercentage(2.8)}}> ${product_item.price} X {product_item.qty} </Text>
                              </View>
                            </TouchableOpacity>
                          );
                        })
