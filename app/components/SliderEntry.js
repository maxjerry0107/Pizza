      ...
      <View style={{ flex: 1, backgroundColor: "#000", height: "100%" }}>
        ...
        >
          ...
          >
            ...
            <Text style={{ fontSize: RFPercentage(2.7), marginBottom: 10 }}>
              {this.convertText(item.description)}
            </Text>
            ...
            >
              {item.Prices.map((priceitem, key) => {
                return (
                  <Option key={key} value={priceitem.attributes_id}>
                    {this.convertText(priceitem.sizes)}{" "}
                    {this.convertText(priceitem.size)} ${priceitem.price}
                  </Option>
                );
              })}
            </Select>
