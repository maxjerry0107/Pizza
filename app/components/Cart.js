        ...
        <View style={{flex:1,}}>
          ...
          <ImageBackground source={require('../assets/images/sign/bg.png')} style={{width:'100%', height:'100%', alignItems:'center', paddingTop:0, resizeMode:'repeat'}}>
            <SafeAreaView style={{width:'100%', flex:1, alignItems:'center', paddingTop:0,}}>
              <TouchableOpacity style={{width:'100%', height:RFPercentage(10),alignItems:'center', }} activeOpacity={0.8} onPress={()=>NavigationService.navigate("Intro")}>
                  <Image source={require('../assets/images/menu/menu_header.png')} style={{height:'80%', width:'70%',}} resizeMode={"contain"}></Image>
                  <Text style={{color:'#000',fontSize:RFPercentage(3), position:'absolute', bottom:0, fontWeight:'bold'}}>CART</Text>
              </TouchableOpacity> 
...
{/*               
             ...
             */}
             <View style={{flexDirection:'row', alignItems:'center', backgroundColor:'#eeffff', height:RFPercentage(5)}}>
                <TouchableOpacity style={{flex:1, flexDirection:'row', alignItems:'center', alignContent:'center', justifyContent:'center'}} activeOpacity={0.7} onPress={()=>{
                  ...
                  {(this.state.signup.type != "pickup" || global.signup ==undefined)&&
                  ...
                  <Text style={[{ fontSize: RFPercentage(3),},this.state.signup.type != "pickup"?{color: '#f00', }:{color:'#00a651',}]}>Pick Up</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{flex:1, flexDirection:'row', alignItems:'center', alignContent:'center', justifyContent:'center'}}activeOpacity={0.7} onPress={()=>{
                  NavigationService.navigate("DeliverySign", {return:"cart"});
                }}>
                <View style={{width:'30%', height:'100%'}}>
                ...
                <Image source={require('../assets/images/sign/car_green.png')} style={{height:'70%',width:'100%'}} resizeMode={'contain'}></Image>}
                 {(this.state.signup.type != "delivery" || global.signup ==undefined) &&
                  <Image source={require('../assets/images/sign/car.png')} style={{width:'100%', height: '80%',}} resizeMode={'contain'}></Image>}
                </View>
                <Text style={[{fontSize:RFPercentage(3),},this.state.signup.type != "delivery"?{color: '#f00', }:{color:'#00a651',}]}>DELIVERY</Text>
                </TouchableOpacity>
              </View>
              {this.state.signup.type == "delivery"&&
              <View style={{width:'100%', paddingVertical:5,paddingHorizontal:10, flexDirection:'row',  backgroundColor:'#fff',alignItems:'center'}}>
                  <Icon type="FontAwesome" name='address-card' style={{fontSize:RFPercentage(2.8),}}></Icon>
                  <Text style={{ textAlign:'left', color:'#000', fontSize:RFPercentage(2.8), marginLeft:10}}>{this.state.signup.data.street+" "+this.state.signup.data.address+" "+this.state.signup.data.city+" "+this.state.signup.data.state}</Text>
              </View>}
              ...
              <View style={{width:'100%', height:RFPercentage(8), backgroundColor:'#43bc55',borderRadius:8, flexDirection:'row'}}>
                <TouchableOpacity onPress={()=>this.onGotoOrderDetail()} style={{flex:1, padding:5, flexDirection:'row', justifyContents:'center'}}>
                  <View  style={{backgroundColor:'#44aa44', width:RFPercentage(7), justifyContent: 'center', borderRadius:3,}}>
                    <Text style={{color:'#fff', fontSize:RFPercentage(2.5), textAlign:'center', textAlignVertical:'center',}}>{this.state.mycart.length}</Text>
                  </View>                  
                  <View style={{flex:1,justifyContent: 'center',}}>
                    <Text style={{ color:'#fff', fontSize:RFPercentage(2.5), textAlign:'center', textAlignVertical:'center',  }}>Checkout</Text>
                  </View>
                  <View  style={{backgroundColor:'#44aa44', justifyContent: 'center', borderRadius:3,}}>
                    <Text style={{color:'#fff', fontSize:RFPercentage(2.2), textAlign:'center', textAlignVertical:'center',  paddingHorizontal:5 }}>${this.getTotalCost()}</Text> 
                  </View>
                </TouchableOpacity>
              </View>
            </SafeAreaView>
