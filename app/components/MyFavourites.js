...
export default class MyFavourites extends Component { 
  ...
  getSmokeFavourite() {
    ...
    smoke_api.get('?req=get-favourites&phonenum=' + global.myphonenumber)
      .then(response => {
          ...
          <ImageBackground source={require('../assets/images/sign/bg.png')} style={{width:'100%', height:'100%', resizeMode:'repeat'}}>
            <SafeAreaView  style={{width:'100%', flex:1, paddingTop:0,}}>
              <TouchableOpacity style={{width:'100%', height:RFPercentage(10),alignItems:'center', }} activeOpacity={0.8} onPress={()=>NavigationService.navigate("Intro")}>
                ...
                <Text style={{color:'#000',fontSize:RFPercentage(3), position:'absolute', bottom:0, fontWeight:'bold'}}>My Favorites</Text>
              </TouchableOpacity> 
              ...
              <ScrollView style={{flex:1, flexDirection:'column', paddingTop:10,}}>
                {
                  this.state.favourites.map ((item,key)=>{
                    return (
                      <TouchableOpacity 
                        style={{flex:1,flexDirection:'row', padding:10,marginBottom:10,backgroundColor:'rgba(255,255,255,0.6)', borderColor:'#888', borderBottomWidth:0.5, borderTopWidth:0.5}}  key={key}
                        onPress={()=>NavigationService.navigate("Product",{product_id:item.products_id, type:item.product_type})}>
                        <Image source={{uri:item.img}} style={{width:'25%',aspectRatio:1,}}></Image>
                        <View style={{flex:1, flexDirection:'column'}}>
                        <Text style={{textAlign:'left', fontSize:RFPercentage(2.8), paddingLeft:10, textAlignVertical:'center'}}> {item.name}</Text>
                        {item.stock!=undefined&&
                        <Text style={{textAlign:'left', fontSize:RFPercentage(2.8), paddingLeft:10, textAlignVertical:'center'}}> In Stock : {item.stock}</Text>}
                        </View>
                      </TouchableOpacity>
                    );
                  })
                  ...
                  this.state.smoke_favourties.map ((item,key)=>{
                    return (
                      <TouchableOpacity activeOpacity={0.7}
                        style={{flex:1,flexDirection:'row', padding:10, marginBottom:10,backgroundColor:'rgba(255,255,255,0.6)', borderColor:'#888', borderBottomWidth:0.5, borderTopWidth:0.5}}  key={key}
                        onPress={()=>NavigationService.navigate("Product",{product_id:item.products_id, type:2})}>
                          <Image source={{uri:item.img}} style={{width:'25%',aspectRatio:1,}}></Image>
                          <View style={{flex:1, flexDirection:'column'}}>
                          <Text style={{textAlign:'left', fontSize:RFPercentage(2.8), paddingLeft:10, textAlignVertical:'center'}}> {item.name}</Text>
                          <Text style={{textAlign:'left', fontSize:RFPercentage(2.8), paddingLeft:10, textAlignVertical:'center'}}> In Stock : {item.stock}</Text>
